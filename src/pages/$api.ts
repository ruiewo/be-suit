...
import type { Methods as Methods5 } from './api/user/me';
...
const api = <T>({ baseURL, fetch }: AspidaClient<T>) => {
  ...
  const PATH5 = '/api/user/me';
  const PATH6 = '/equipment';
  const GET = 'GET';
  const POST = 'POST';
    ...
    api: {
      category: {
        _code_: (val2: number | string) => {
          ...
          return {
            get: (option?: { config?: T | undefined } | undefined) => fetch<Methods0['get']['resBody']>(prefix, prefix2, GET, option).json(),
            $get: (option?: { config?: T | undefined } | undefined) =>
              fetch<Methods0['get']['resBody']>(prefix, prefix2, GET, option)
                .json()
                .then(r => r.body),
            $path: () => `${prefix}${prefix2}`,
          };
      ...
      user: {
        me: {
          get: (option?: { config?: T | undefined } | undefined) => fetch<Methods5['get']['resBody']>(prefix, PATH5, GET, option).json(),
          $get: (option?: { config?: T | undefined } | undefined) =>
